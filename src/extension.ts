...
export function activate(context: vscode.ExtensionContext) {
  const customWords: string[][] = vscode.workspace.getConfiguration('toggler').get<string[][]>('words');

  assert(customWords.constructor === Array, 'Custom words in togger.words setting is not a valid array');

  const builtWords: string[][] = Object.assign(defaults, customWords);
 ...
 */
export function findReplacementText(incomingWords: string, switchableWords: string[][]) {
